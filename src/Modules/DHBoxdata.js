// Please add Data for new page before using 
import taskbasedlearning from '../assets/taskbasedlearning.jpg'
import learningpaths from '../assets/learningpaths.jpg'
import dailyquizes from '../assets/dailyquizes.jpg'
import Mentorship from '../assets/Mentorship.png'
import Project_based_learning from '../assets/Project_based_learning.png'
import Tech_tags from '../assets/Tech_tags.png'
import Streak_counter from '../assets/Streak_counter.png'
import Skill_tracking_analytics from '../assets/Skill_tracking_and_analytics.png'
export const data =[
{H1:"Task Based Learning", 
desc:"Enhance your skills with tasks ranging from basic to advanced.",
img: taskbasedlearning
},
{H1:"Daily Quizes", 
    desc:"Test your knowledge and earn points with daily quizzes.",
    img: dailyquizes
},
{H1:"Comprehensive Learning Paths", 
desc:"Follow structured paths to master web development technologies.",
img: learningpaths
},

// Achievements & Rewards

{H1:"Earn Tech Tags", 
    desc:"Maintain daily activity streaks to unlock special rewards.",
    img:Tech_tags
    },
    {H1:"Streak Counter", 
        desc:"Follow structured paths to master web development technologies.",
        img: Streak_counter
        },
        {
            H1:"Skill-Tracking & Analytics",
            desc:"Track your progress, set goals, and identify areas for improvement",
            img:Skill_tracking_analytics
        },
        // Real-Life Projects
        {
            H1:"Poject-Based Learning",
            desc:"Work on real-world projects to build your portfolio",
            img:Project_based_learning
        },
        {
            H1:"Feedback & Mentorship",
            desc:"Receive guidance and feedback from experienced developers.",
            img:Mentorship
        }

]
